...
use hqe_protocol::models::{WorkflowDefinition, WorkflowStep, MCPToolDefinition};
use serde_json::json;
...
async fn test_flow_execution() -> anyhow::Result<()> {
    ...
    let mut engine = FlowEngine::new(registry.clone());
    ...
    registry.register_tool(
        ...
        Box::new(|args| {
            ...
            Box::pin(async move {
                let ticker = args.get("ticker").and_then(|s| s.as_str()).unwrap_or("UNKNOWN");
                Ok(json!({
                    "symbol": ticker,
                    "price": 150.00
                }))
